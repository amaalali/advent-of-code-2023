...
def parseLRs(input: String): Iterator[Dir] =
  scala.io.Source
    .fromString(input)
    .getLines()
    .take(1)
    ...
    .head
    .toList
    .pipe(_.asInstanceOf[List[Dir]])
    .pipe(Iterator.continually)
    .flatten

val linePattern = """(?<key>\w+) = \((?<left>\w+),\s?(?<right>\w+)\)""".r
def parseMapping(line: String): Map[String, String] = {
  val matches = linePattern.findFirstMatchIn(line).get

  Map(
    s"${matches.group("key") + L}" -> matches.group("left"),
    s"${matches.group("key") + R}" -> matches.group("right")
  )
}
