...
val debug = false
def logDebug(msg: => String, marker: String = ""): Unit =
  if (debug)
    println(s"[DEBUG${if (marker.isEmpty) "" else " " + marker} $msg")

// case class S(x: Int)
object S:
  val symbols = Vector('*', '#', '+', '$', '&', '%', '/', '-')
  def isSymbol(char: Char): Boolean = symbols.contains(char)

case class Coord(x: Int, y: Int):
  def isAdjacent(other: CoordRng): Boolean =
    other.range
      .find(otherCoord => (x.isPoM1(otherCoord.x)) && (y.isPoM1(otherCoord.y)))
      .isDefined

extension (l: Int)
  def isPoM1(r: Int): Boolean =
    (l == r) ||
      ((l - 1) == r) ||
      ((l + 1) == r)

extension (c: Char)
  def toInt: Int = c.toString().toInt
  def isInt: Boolean = c.toString.toIntOption.isDefined

case class CoordRng(start: Int, end: Int, lineNum: Int):
  def range: Seq[Coord] = (start to end).map(Coord(_, lineNum))

// def locationOfSymbols(lsS: List[S], lineNum: Int): Set[Coord] =
//   lsS.map(x => Coord(x.x, lineNum)).toSet

case class N(value: Int, location: CoordRng)

case class I_N(value: String, start: Int) {
  def add(c: Char): I_N = copy(value = value + c)
  def end(nd: Int, lineNum: Int): N = N(value.toInt, CoordRng(start, nd, lineNum))
}

def parseLine(lineLength: Int)(input: String, lineNumber: Int): (Set[N], Set[Coord]) = {
  def endOfLine(pos: Int): Boolean = {
    pos == (lineLength - 1)
  }

  val (ns: Set[N], ss: Set[Coord], x) =
    input.zipWithIndex.foldLeft(Set.empty[N], Set.empty[Coord], Option.empty[I_N]) { case ((_ns, _sym, incompleteN), (nextChar, xIndex)) =>
      (nextChar, incompleteN) match
        case (c, None) if c.isInt && endOfLine(xIndex)      => (_ns + N(c.toInt, CoordRng(xIndex, xIndex, lineNumber)), _sym, None)
        case (c, None) if c.isInt                           => (_ns, _sym, Some(I_N(c.toString, xIndex)))
        case (c, Some(i_n)) if c.isInt && endOfLine(xIndex) => (_ns + i_n.add(c).end(xIndex, lineNumber), _sym, None)
        case (c, Some(i_n)) if c.isInt                      => (_ns, _sym, Some(i_n.add(c)))

        case ('.', None)      => (_ns, _sym, None)
        case ('.', Some(i_n)) => (_ns + i_n.end(xIndex - 1, lineNumber), _sym, None)

        case (c, None)      => (_ns, _sym + Coord(xIndex, lineNumber), None)
        case (c, Some(i_n)) => (_ns + i_n.end(xIndex - 1, lineNumber), _sym + Coord(xIndex, lineNumber), None)

        case x => throw new NotImplementedError(s"Missing match for case: ${x}")
    }

  (ns, ss)
}

def filterAdjacent(ns: Set[N], symbols: Set[Coord]): Set[N] =
  ns.filter(n => symbols.find(y => y.isAdjacent(n.location)).isDefined)
...
def run1(input: String) = {
  ...
  scala.io.Source
    ...
    .zipWithIndex
    .sliding(3)
    .tapEach(x => println(s"[DEBUG window] $x"))
    .map(
      _.map(parseLine(lineLength))
        .foldLeft((Set.empty[N], Set.empty[Coord])) { case ((_ns, _ss), (n, s)) =>
          (_ns ++ n, _ss ++ s)
        }
    )
    // .tapEach(x => println(x._2.toList.groupBy(_.y).mapValues(_.sortBy(_.x)).toMap))
    .flatMap(filterAdjacent)
    .toList
    .groupBy(_._2.lineNum)
    .mapValues(_.sortBy(_.location.start).toList)
    .toMap
  // .foldLeft(Set.empty[N])((acc, next) => acc + next)
  // .tapEach(x => logDebug(x.toString, "set"))
  // .map(_.value)
  // .sum
}
...
println("\n")
println(">" * 10)
// println(parseLine("467..114..", 1))
// println(parseLine("...*......", 1))
// println(parseLine(".....+.58.", 1))
// test(run1(example), 4361)
// test.where(run1(puzzle), _ > 336127)
// test.where(run1(puzzle), _ != 338506)
// test.where(run1(puzzle), _ != 338464)
// println(run1(puzzle))
// println(run1(puzzle)(1))

println(debugSample)
println(run1(debugSample))
// run1(puzzle)
println("<" * 10)
println("\n")

Map(
  0 -> List(N(2, CoordRng(1, 1, 0)), N(52, CoordRng(3, 3, 0))),
  1 -> List(N(5, CoordRng(1, 1, 1)), N(54, CoordRng(3, 3, 1))),
  2 -> List(N(7, CoordRng(1, 1, 2)), N(57, CoordRng(3, 3, 2)))
)
def debugSample: String =
  """.1*2
    |.3*4
    |.5*6
    |.7*8""".stripMargin
...
def puzzle: String =
  """...766.......821.547.....577......................................387.....................56..........446.793..........292.................*
    |...........................%...../.....981..........627..../..........-.....623......610..-..............*..................16......891....1
    |...$...........716..&336.......470.325.................*.84........$..34....*.....+.....#.....*76....#.........303.433........-........&....""".stripMargin /*
    ...
    |..808..............................365..................195.........................90......482.837............................404.214......""".stripMargin
