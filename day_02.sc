...
/** Bag with cubes - red green blud hide secret bumber of cubes in bag figure out information about the number of cubes
  *
  * random sampling (handful)
  *
  * record info about sampling game ID,
  */

case class H(r: Int, g: Int, b: Int)
object H {
  val INPUT = H(12, 13, 14)

  given ordering: Ordering[H] = new Ordering[H] {
    override def compare(x: H, y: H): Int =
      if (x.r > y.r || x.g > y.g || x.b > y.b) 1
      else -1 // (x.r <= y.r && x.g <= y.g && x.b <= y.b)
  }
}

case class G(id: Int, handfuls: Seq[H])

val gameCapture = """Game (?<id>\d+):""".r
val redCapture = """(?<count>\d+) red""".r
val greenCapture = """(?<count>\d+) green""".r
val blueCapture = """\s+(?<count>\d+) blue""".r

def parseHandful(handful: String): H =
  val red = redCapture
    .findFirstMatchIn(handful)
    .map(_.group("count").toInt)
    .getOrElse(0)
  val green = greenCapture
    .findFirstMatchIn(handful)
    .map(_.group("count").toInt)
    .getOrElse(0)
  val blue = blueCapture
    .findFirstMatchIn(handful)
    .map(_.group("count").toInt)
    .getOrElse(0)
  H(red, green, blue)

def parseLine(line: String): G =
  val id = gameCapture.findFirstMatchIn(line).map(_.group("id")).get.toInt
  val indexOfColon = line.indexOf(':')
  val handfulsString = line.slice(indexOfColon + 1, line.length()).split(';')
  val handfuls = handfulsString.map(parseHandful)
  G(id, handfuls)

def isValidHandful(g: G): Boolean =
  import math.Ordered.orderingToOrdered
  g.handfuls.forall(_ < H.INPUT)
...
def run1(input: String): Int =
  Source
    ...
    .getLines()
    .map(parseLine)
    .filter(isValidHandful)
    .map(_.id)
    .tapEach(x => println(s"[DEBUG] $x"))
    .sum

def minValidForHandful(handfuls: Seq[H]): H =
  H(
    handfuls.map(_.r).max,
    handfuls.map(_.g).max,
    handfuls.map(_.b).max
  )

def power(h: H): Int = h.r * h.g * h.b
...
def run2(input: String): Int =
  Source
    ...
    .map(_.handfuls)
    // .tapEach(x => println(s"[DEBUG] $x"))
    .map(minValidForHandful andThen power)
    .sum

/** >>>>>>>>> TESTING >>>>>>>>>
  */

def test[A](result: A, expected: => A): Unit = println(s"""Test ${
...
//   expected = G(1, Seq(H(3, 0, 4), H(1, 2, 6), H(0, 2, 0)))
// )

// test(isValidHandful(parseLine(line = line1)), true)
// test(isValidHandful(parseLine(line = line3)), false)

// test(run1(example), 8)
// println(run1(puzzle))

test(run2(example), 2286)
println(run2(puzzle))
...
def puzzle =
  """Game 1: 8 green, 4 red, 4 blue; 1 green, 6 red, 4 blue; 7 red, 4 green, 1 blue; 2 blue, 8 red, 8 green
    ...
    |Game 100: 5 green, 17 blue, 5 red; 15 blue; 13 green, 8 red, 3 blue; 16 blue, 15 green, 8 red; 16 green, 2 blue, 3 red""".stripMargin
