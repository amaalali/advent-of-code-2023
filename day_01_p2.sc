...
val spellingMappingForward = Map(
  // "zero" -> 0,
  "one" -> 1,
  "two" -> 2,
  "three" -> 3,
...
val spellingMappingReversed =
  spellingMappingForward.map((k, v) => (k.reverse, v))

def parseForward(string: String): Int =
  firstIntOrSpelling(string, spellingMappingForward)

def parseBackward(string: String): Int =
  firstIntOrSpelling(string.reverse, spellingMappingReversed)

def firstIntOrSpelling(searchSpace: String, mapping: Map[String, Int]): Int =
  val firstSpelling =
    mapping
      .map(pair => (searchSpace.indexOf(pair._1), pair._2))
      .filter(_._1 >= 0)
      .minByOption(_._1)

  val firstDigit =
    """\d""".r
      .findFirstIn(searchSpace)
      .map(digit => (searchSpace.indexOf(digit), digit.toInt))

  (firstSpelling, firstDigit) match
    case (None, Some((_, r)))                               => r
    case (Some((_, r)), None)                               => r
    case (Some((i_s, r_s)), Some((i_d, r_d))) if i_s < i_d  => r_s
    case (Some((i_s, r_s)), Some((i_d, r_d))) if i_d <= i_s => r_d
    case _                                                  => ??? // Unreachable as long as input is well formed

def tupleFork[A, F, G](f: A => F, g: A => G)(a: A): (F, G) = (f(a), g(a))

def formNumber(ints: (Int, Int)): Int =
  (ints.head.toString + ints.last.toString).mkString.toInt

def run(string: String): Int =
  Source
    .fromString(string)
    .getLines()
    .map(tupleFork(parseForward, parseBackward) andThen formNumber)
    .sum

val examplExpectedResult = 281 + 18

println("\n")
println(">" * 10)
println(run("oneightoneight"))
val exampleResult = run(example)
println(
  s"Example result: passed=${exampleResult == examplExpectedResult} expected=${examplExpectedResult} got=${exampleResult}"
)
println(s"Puzzle result is ${run(input)}")
println("<" * 10)
println("\n")
...
def input =
  """5ffour295
    ...
    |56four4one2
    |cqmzqbxzfvonevmmmlxsnjr5zfg""".stripMargin('|')
